<template lang="pug">
	li(v-if="changeable")
		//- role="button": der Anker führt nirgendwohin, er öffnet einen Dialog.
		//- Ohne die Rolle meldet eine Sprachausgabe "Verweis" und die Leertaste tut
		//- nichts (den Tastendruck ergänzt der Kern, OCO.wireButtonAnchors).
			...
			.uk-modal-dialog
				...
				.uk-modal-body
					...
					.uk-alert-danger(v-if="!valid && valid != undefined", uk-alert)
						p.uk-text-center {{ t('The API-Key is invalid!') }}
				.uk-modal-footer
					button.uk-button.uk-button-default.uk-modal-close.uk-margin-small-right(type='button') {{ t('Close') }}

					button(v-if="!loading", type='button', @click="setKey", :disabled="loading").uk-button.uk-button-primary.uk-align-right {{ t('Save') }}
